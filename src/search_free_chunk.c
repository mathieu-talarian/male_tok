...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/01/15 18:56:36 by mmoullec          #+#    #+#             */
/*   Updated: 2019/01/15 19:00:40 by mmoullec         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "malloc.h"

void		*fusion_block(t_zone *current_zone, t_chunk *chunk, int t, \
		size_t size)
{
	t_chunk	*tmp;
...
void		build_new_chunk(t_zone **current_zone, \
		t_chunk **current_chunk, size_t size)
{
	t_chunk	*new;
...
void		*split_block(t_zone *current_zone, t_chunk *current_chunk, \
		int t, size_t size)
{
	size_t	res;
...
void		*search_free_chunk(t_zone *zone, int t, size_t size)
{
	...
	while (current_zone)
	{
		...
		return ((*indirect) ?
				split_block(current_zone, (*indirect), t, size) :
